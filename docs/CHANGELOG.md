...
### ✨ Added
- **Group-Based Targeting**: Join questions can be limited to selected user groups per space
- **Group Selection Settings**: New settings UI to choose which groups must answer questions

### 🔧 Changed
- **Join Question Flow**: Questions are shown only to users in selected groups
- **Default Behavior**: If no groups are selected, no join questions are shown
