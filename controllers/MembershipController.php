...
class MembershipController extends SpaceController
{
    ...
    public function beforeAction($action)
    {
        ...
        // Check if user is space admin for admin actions
        if (in_array($action->id, ['index']) && !$this->contentContainer->isAdmin()) {
    ...
    public function actionCancel()
    {
        ...
        try {
            // Explicitly delete associated answers first
            $answersDeleted = SpaceJoinAnswer::deleteAll(['membership_id' => $membership->id]);
            
            // Delete the membership (this should trigger the EVENT_BEFORE_DELETE event as well)
            if ($membership->delete()) {
                $transaction->commit();

                // Send email notification after transaction is committed
                
                Yii::info("Application cancelled successfully. Deleted {$answersDeleted} answers for membership ID {$membership->id}", 'spaceJoinQuestions');
