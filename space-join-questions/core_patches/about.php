...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        <?php 
        // Space Join Questions Module: Display application status for current user
        if (!Yii::$app->user->isGuest && $space->getSetting('requireApplication', 'spaceJoinQuestions')):
            $membership = $space->getMembership(Yii::$app->user->id);
            if ($membership && $membership->status == \humhub\modules\space\models\Membership::STATUS_APPLICANT):
        ?>
            <div class="row">
                <div class="col-md-12">
                    <div class="alert alert-info">
                        <i class="fa fa-clock-o"></i>
                        <strong><?= Yii::t('SpaceJoinQuestionsModule.base', 'Application Status') ?></strong>
                        <br>
                        <?= Yii::t('SpaceJoinQuestionsModule.base', 'Your application is pending review.') ?>
                        <small>(<?= Yii::$app->formatter->asDate($membership->created_at) ?>)</small>
                    </div>
                </div>
            </div>
        <?php 
